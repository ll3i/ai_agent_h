...
def build_prompt(strict: bool = False) -> str:
    ...
    header = """
You are a HIGHLY ACCURATE IC component quality inspection expert. 
...
1️⃣ component_missing: Component body completely absent (empty footprint only)
...
2️⃣ lead_connectivity_fail ⭐⭐⭐ HIGHEST PRIORITY ⭐⭐⭐
   ...
   DETAILED INSPECTION FOR EACH LEAD (3 leads total):
   ■ Check 1: Can see lead clearly?
   ...
   ✗ FALSE: ALL leads PASS all 5 checks
   ✓ TRUE: ANY lead FAILS any check
...
"""
    ...
    if strict:
        rule = """
【VERIFICATION MODE - 정밀 재검토 (Reduce False Positives)】
⚠️  GOAL: ELIMINATE FALSE POSITIVES. When in doubt, it is NORMAL.
⚠️  CRITICAL: Distinguish SHADOWS from BROKEN LEADS.
    - Dark/Black area on lead path = SHADOW (Normal) -> NOT a break.
    - Lead looks faint/blurry = LIGHTING ARTIFACT (Normal) -> NOT a break.
    - Only mark lead_connectivity_fail if there is a CLEAR, BRIGHT GAP.

⚠️  Rule: If lead path is continuous but dark -> CONNECTED (Normal).
⚠️  Rule: If body has scratches/chips but no internal exposure -> INTACT (Normal).
"""
    else:
        rule = """
    ...
    for attempt in range(max_retries):
        try:
            ...
            if label2 == LABEL_ABNORMAL:
                # ☢️ NUCLEAR OPTION: If only 1 defect found in Verify, assume it's a Shadow/FP
                # This sacrifices Recall for Precision (to fix the massive FP rate)
                if reasoning2['total_defects'] == 1:
                     verification = verify(LABEL_NORMAL, reasoning2, is_review=True)
                     logger.info(f"Final Decision: {verification['decision_label']} "
                                f"(confidence: {verification['confidence']:.2f}, FORCED NORMAL (Single Defect Policy))")
                     logger.info("=" * 60 + "\n")
                     return {
                        ...
                        'verified': True,
                        'iterations': 2,
                        'reason': 'Forced Normal (Single Defect in Verify)'
                     }
